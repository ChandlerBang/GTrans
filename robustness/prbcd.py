...
class PRBCD:
    ...
    def pretrain_model(self):
        ...
        filename = f'saved/arxiv_GCN_s0.pt'
        model.load_state_dict(torch.load(filename, map_location='cuda'))
        # model.fit(data, train_iters=500, patience=1000, verbose=True)
        model.eval()
    ...
    def attack_graph(self, edge_index=None, edge_weight=None, lr_adj=0.1, ptb_rate=0.1):
        data = self.data
        epochs = self.epochs
        model = self.model
